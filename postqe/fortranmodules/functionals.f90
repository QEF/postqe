...
! - #ifdef __LIBXC removed
! - USE KINDS only DP substituted with INTEGER, PARAMETER :: DP = selected_real_kind(14,200)
! - USE CONSTANTS... substituted with used constants in each subroutine
! - other minor changes
! All functionals from espresso5.4.0 were kept
!-----------------------------------------------------------------------
...
subroutine c09x (rho, grho, sx, v1x, v2x)
  ...
  REAL(DP), PARAMETER :: c1 = 0.75d0 / pi
  REAL(DP), PARAMETER :: c2 = 3.093667726280136d0
  REAL(DP), PARAMETER :: c5 = 4.d0 * third
...
subroutine b86b (rho, grho, iflag, sx, v1x, v2x)
  ...
  REAL(DP), PARAMETER :: c2 = 3.093667726280136d0, c5 = 4.d0 * third
  ! parameters of the functional
  real(DP) :: k (3), mu(3)
...
subroutine perdew86 (rho, grho, sc, v1c, v2c)
  ...
  REAL(DP), PARAMETER :: p1 = 0.023266d0
  REAL(DP), PARAMETER :: p2 = 7.389d-6
  REAL(DP), PARAMETER :: p3 = 8.723d0
  REAL(DP), PARAMETER :: p4 = 0.472d0
  REAL(DP), PARAMETER :: pc1 = 0.001667d0
  REAL(DP), PARAMETER :: pc2 = 0.002568d0
  REAL(DP), PARAMETER :: pci = pc1 + pc2
  REAL(DP), PARAMETER :: third = 1.d0 / 3.d0
...
subroutine ggac (rho, grho, sc, v1c, v2c)
  ...
  INTEGER, PARAMETER :: DP = selected_real_kind(14,200)
  real(DP) :: rho, grho, sc, v1c, v2c
  REAL(DP), PARAMETER :: al = 0.09d0, pa = 0.023266d0, pb = 7.389d-6
  REAL(DP), PARAMETER :: pc = 8.723d0, pd = 0.472d0, cx = -0.001667d0
  REAL(DP), PARAMETER :: cxc0 = 0.002568d0, cc0 = - cx + cxc0
  REAL(DP), PARAMETER :: third = 1.d0 / 3.d0, pi34 = 0.6203504908994d0
  REAL(DP), PARAMETER :: nu = 15.755920349483144d0, be = nu * cc0
...
subroutine pbex (rho, grho, iflag, sx, v1x, v2x)
  ...
  REAL(DP), PARAMETER :: c2 = 3.093667726280136_DP, c5 = 4._DP * third
  REAL(DP), PARAMETER :: c6 = c2*2.51984210, c7=5._DP/6._DP, c8=0.8_DP ! (3pi^2)^(1/3)*2^(4/3) 
  ! parameters of the functional
...
subroutine pbex_vec (rho, grho, iflag, sx, v1x, v2x, length, small)
  ...
  ! numerical coefficients (NB: c2=(3 pi^2)^(1/3) )
  REAL(DP), PARAMETER :: third = 1._DP / 3._DP, c1 = 0.75_DP / pi 
  REAL(DP), PARAMETER :: c2 = 3.093667726280136_dp, c5 = 4.0_dp * third
  ! parameters of the functional
  real(DP) :: k (3), mu(3)
...
subroutine wcx (rho, grho, sx, v1x, v2x)
  ...
  REAL(dp), PARAMETER:: third = 1.d0 / 3.d0, c1 = 0.75d0 / pi 
  REAL(dp), PARAMETER:: c2 = 3.093667726280136d0, c5 = 4.d0 * third
  REAL(dp), PARAMETER:: teneightyone = 0.123456790123d0
  ! parameters of the functional
  real(DP) :: k, mu, cwc
