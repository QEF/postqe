...
def spherical_1d(charge, G, a, x0, e1, nx):
    """
        ...
        for y in range(0, nr[1]):
            for z in range(0, nr[2]):
                if x == 0 and y == 0 and z == 0:    # at the Gamma point
                    ...
                    rho0g = fft_charge[x, y, z] * \
                        complex(np.cos(arg), np.sin(arg))  # Move the origin to x0
...
def fft_interp_2d(charge, G, a, x0, e1, e2, nx, ny):
    # normalize e1
    m1 = np.linalg.norm(e1)
    if abs(m1) < 1.0E-6:  # if the module is less than 1.0E-6
        e1 = a[0]
    ...
    for x in range(0, nr[0]):
        for y in range(0, nr[1]):
            for z in range(0, nr[2]):
                ...
                for i in range(0, nx):
                    np.exp(np.complex(0, 2.0 * pi * i * deltax * np.dot(e1, G[x, y, z]))
                           + np.dot(x0, G[x, y, z]))
...
def plot_1d_charge(charge, G, struct_info, x0=(0, 0, 0), e1=(1, 0, 0), nx=20, ylab='charge',
                   plot_file='', method='FFT', format=''):
    """
    This function calculates a 1D plot of the input charge (or else), starting from the
    input point x0 and along the direction given by the vector e1. The G vectors
    in the reciprocal space must also be given in input. nx is the number of
    ...
    :param method: interpolation method. 'spherical' is for averaged spherical, \
...
def polar_2d(charge, G, nx, ny, radius, alat):
    """
...
def plot_2d_charge(charge, G, struct_info, x0=(0, 0, 0), e1=(1, 0, 0), e2=(0, 1, 0),
                   nx=20, ny=20, radius=1, zlab='charge', plot_file='',
                   method='FFT', format=''):
    """
    This function calculates a 2D plot of the input charge (or else).
    If method=='FFT', the plot starts from the input point x0 and along the
    directions given by the vectors e1, e2. These vectors define the section
    plane along which the plot is draw. If method=='polar', the plot is a polar
    one on a sphere of radius = 'radius'. The G vectors in the reciprocal space
    must also be given in input. nx and ny define the grid of points where the
    charge is effectively calculated using Fourier interpolation (or polar projection).

    :param charge: eletronic charge density (or other quantity) to be plotted
    :param G: G vectors in the reciprocal space
    :param struct_info: a dictionary with structural info on the unit cell
    :param x0: 3D vector, origin of the line
    :param e1: 3D vector which determines the plotting plane
    :param e2: 3D vector which determines the plotting plane
    :param nx: number of points along e1
    :param ny: number of points along e2
    :param zlab: y axix label in the plot
    :param plot_file: if plot_file!='', write the plotting values on a text file
    :param method: interpolation method ('FFT' or 'splines' or 'polar', only 'FFT' \
    and 'polar' implemented)
...
def fft_interp_3d(charge, G, a, x0, e1, e2, e3, nx, ny, nz):
    # normalize e1
    m1 = np.linalg.norm(e1)
    if abs(m1) < 1.0E-6:  # if the module is less than 1.0E-6
        e1 = a[0]
    ...
    for x in range(0, nr[0]):
        for y in range(0, nr[1]):
            for z in range(0, nr[2]):
                ...
                eigx = np.zeros(nx, dtype=complex)
                for i in range(0, nx):
                    eigx[i] = np.exp(np.complex(0, 2.0 * pi * i * deltax * np.dot(e1, G[x, y, z]))
                                     + np.dot(x0, G[x, y, z]))
...
def plot_3d_charge(charge, G, struct_info, x0=(0, 0, 0), e1=(1, 0, 0), e2=(0, 1, 0), e3=(0, 0, 1),
                   nx=20, ny=20, nz=20, zlab='charge', plot_file='', method='FFT', format=''):
    try:
        from compute_vs_cython import FFTinterp3D_Cython
        X, Y, Z, W = FFTinterp3D_Cython(charge, G, struct_info['a'], x0, e1, e2, e3, nx, ny, nz)
    except ImportError:
        X, Y, Z, W = fft_interp_3d(charge, G, struct_info['a'], x0, e1, e2, e3, nx, ny, nz)
    pass

    write_3Dcharge_file(X, Y, Z, W, struct_info, x0, e1, e2, e3,
                        nx, ny, nz, plot_file, method, format)
...
def multiple_plot_xy(x, y, xlabel="", ylabel="", labels=""):
    ...
    if labels == '':
        ...
        except Exception:  # if not use a single x axis
            for i in range(0, len(y[0, :])):
