...
"""
A tentative collection of functions for writing the charge in different formats
(to be integrated into the charge class).
"""
import numpy as np
...
def write_1Dcharge_file(X, Y, nx=1, plot_file='chargeplot1D.out'):
    ...
    f = open(plot_file, 'w')
    f.write(' '.join([
        '# Minimum, maximum, imaginary charge: ',
        "{:.9E}  ".format(charge_min),
        "{:.9E}  ".format(charge_max),
        "{:.9E}\n".format(charge_im)
    ]))
...
def write_2Dcharge_file(X, Y, Z, struct_info, x0, e1, e2, nx=1, ny=1,
                        plot_file='chargeplot2D.out', method='FFT', format='gnuplot'):
    """
    ...
    if format == 'gnuplot':
        f.write(
            '# Minimum, maximum, imaginary charge: ' + "{:.9E}  ".format(charge_min) + "{:.9E}  ".format(charge_max) +
            "{:.9E}\n".format(charge_im))
...
def write_3Dcharge_file(X, Y, Z, W, struct_info, x0, e1, e2, e3, nx=1, ny=1, nz=1,
                        plot_file='chargeplot3D.out', method='FFT', format='gnuplot'):
    """
    Writes a file for a 2D plot of the charge in different formats.
    ...
    deltax = m1 / (nx - 1)  # noqa: F841 (local variable is not used!)
    deltay = m2 / (ny - 1)  # noqa: F841
    deltaz = m3 / (nz - 1)  # noqa: F841

    # Determine max and min of the (real) charge and the sum of imaginary (absolute) charge
    charge_min = np.min(W.real)  # noqa: F841
    charge_max = np.max(W.real)  # noqa: F841
    charge_im = np.sum(np.abs(W.imag)) / nx / ny  # noqa: F841
